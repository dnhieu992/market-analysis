/**
 * BTC SHORT @ 00:00 UTC · TP = entry − $tpPts · force-close 04:00 UTC · no stop · $notional fixed.
 * WITH a 1D BEAR filter: only enter when the most-recently-CLOSED 1D candle (prior day) is in a downtrend.
 *
 * Filters compared vs unfiltered baseline:
 *   - UTBot-1D bear : daily close < UTBot trailing stop (kv=2, ATR10).
 *   - EMA200-1D bear: daily close < EMA200.
 *
 * Usage: ts-node --project apps/api/tsconfig.json scripts/run-btc-short-0400-bearfilter-backtest.ts \
...
const SYMBOL = 'BTCUSDT', ENTRY_HOUR = 0;
const KV = 2, ATR_P = 10, EMA_P = 200, DAY_MS = 864e5;
...
type Res = { trades: number; tpHits: number; net: number; gross: number; forced: number; forcedWin: number };

function run(h1: H1[], d1: D1[], notional: number, feePct: number, tpPts: number, exitHour: number, filter: Filter, winStartMs: number): Res {
  const f = feePct / 100;
  const stop = utBotStops(d1, ATR_P, KV), e = ema(d1, EMA_P);
  const utbear = new Map<number, boolean>(), emabear = new Map<number, boolean>();
  for (let i = 0; i < d1.length; i++) {
    if (stop[i]! > 0) utbear.set(d1[i]!.openTime, d1[i]!.close < stop[i]!);
    if (e[i]! > 0) emabear.set(d1[i]!.openTime, d1[i]!.close < e[i]!);
  }
  const r: Res = { trades: 0, tpHits: 0, net: 0, gross: 0, forced: 0, forcedWin: 0 };
  for (let i = 0; i < h1.length; i++) {
    const cand = h1[i]!;
    if (cand.hour !== ENTRY_HOUR || cand.openTime < winStartMs) continue;
    if (filter !== 'none') {
      const prevDay = cand.openTime - DAY_MS;
      const bear = filter === 'utbot' ? utbear.get(prevDay) : emabear.get(prevDay);
      if (bear !== true) continue;
    }
    const entry = cand.open, tpPx = entry - tpPts;
    let exitRet: number | null = null, hitTP = false;
    for (let j = i; j < h1.length; j++) {
      if (j > i && h1[j]!.hour === exitHour) { exitRet = (entry - h1[j]!.open) / entry; break; }
      if (h1[j]!.low <= tpPx) { exitRet = tpPts / entry; hitTP = true; break; }
      if (j - i > 8) { exitRet = (entry - h1[j]!.close) / entry; break; }
    }
    if (exitRet === null) continue;
    r.trades++; r.gross += notional * exitRet; r.net += notional * ((1 + exitRet) * (1 - f) * (1 - f) - 1);
...
async function main() {
  ...
  const days = Number(daysA ?? 365), fee = Number(feeA ?? 0.05), notional = Number(notA ?? 1000), tpPts = Number(tpA ?? 500), exitHour = Number(ehA ?? 4);
  const endMs = Date.now(), winStartMs = endMs - days * DAY_MS, warmStartMs = winStartMs - 320 * DAY_MS;

  const raw1h = await fetchKlines(SYMBOL, '1h', winStartMs, endMs);
  const raw1d = await fetchKlines(SYMBOL, '1d', warmStartMs, endMs);
  ...
  console.log(`\n=== BTC SHORT @ 00:00 UTC · TP −$${tpPts} · force-close ${String(exitHour).padStart(2, '0')}:00 UTC · NO stop · 1D BEAR filter · ${days}d · $${notional}/trade · fee ${fee}%/side ===\n`);
  console.log('  filter        | trades | TP hit | TP% | forced(green) |  GROSS $  | total fees |   NET $   | net/trade');
  for (const filt of ['none', 'utbot', 'ema200'] as Filter[]) {
    const r = run(h1, d1, notional, fee, tpPts, exitHour, filt, winStartMs);
    ...
    console.log(
      `  ${filt.padEnd(13)} | ${String(r.trades).padStart(6)} | ${String(r.tpHits).padStart(6)} | ${(fmt(tpRate, 0) + '%').padStart(4)} | ` +
        `${(String(r.forced) + '(' + r.forcedWin + ')').padStart(13)} | ${usd(r.gross).padStart(9)} | ${usd(-(r.gross - r.net)).padStart(10)} | ${usd(r.net).padStart(9)} | ${usd(r.trades ? r.net / r.trades : 0).padStart(8)}`,
    );
  }
  console.log('');
}
