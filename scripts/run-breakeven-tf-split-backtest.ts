/**
 * NEW exit rule, threshold SPLIT BY TIMEFRAME:
 *   - H4 legs: once +h4Tp% in profit, move stop-loss to entry (breakeven).
 *   - 1D legs: once +d1Tp% in profit, move stop-loss to entry (breakeven).
 * The trade still exits on the UTBot flip; but if price retraces to entry AFTER arming,
 * the leg is closed at breakeven (≈$0, minus fees) instead of riding the flip.
 *
 * Applied to EVERY leg (base + pullback adds). Live config: pullback add-on only on kv=4 (BNB).
 *
 * CURRENT = ride to flip, no breakeven.  NEW = breakeven stop armed at the per-TF threshold.
 ...
 * ordering avoids same-candle whipsaw). Arming and the BE touch are checked intra-candle.
 *
 * Usage: ts-node --project apps/api/tsconfig.json scripts/run-breakeven-tf-split-backtest.ts [days] [feePctPerSide] [notional] [h4Tp] [d1Tp] [bandPct] [maxAdds]
 */
...
async function main() {
  const [, , daysArg, feeArg, notArg, h4Arg, d1Arg, bandArg, maxArg] = process.argv;
  const days = Number(daysArg ?? 365), fee = Number(feeArg ?? 0.05), notional = Number(notArg ?? 1000);
  const h4Tp = Number(h4Arg ?? 3), d1Tp = Number(d1Arg ?? 5), band = Number(bandArg ?? 1), maxAdds = Number(maxArg ?? 3);
  const beForInterval = (interval: string) => (interval === '1d' ? d1Tp : h4Tp);
  const endMs = Date.now(), startMs = endMs - days * 864e5;

  console.log(`\n=== NEW RULE: move SL to breakeven — H4 at +${h4Tp}%, 1D at +${d1Tp}%, then ride to flip | ${days}d | $${notional}/leg FLAT | fee ${fee}%/side ===`);
  ...
  for (const cfg of LIVE) {
    const beTp = beForInterval(cfg.interval);
    ...
    console.log(`${cfg.symbol} ${cfg.interval} kv=${cfg.kv}${addonEnabled(cfg.kv) ? ' (pullback live)' : ''} — BE at +${beTp}%`);
