...
export function createOkxTradeRepository(client = prisma) {
  return {
    ...
    async deleteClosedBefore(date: Date): Promise<number> {
      const doomed = await client.okxTrade.findMany({
      ...
      const [, res] = await client.$transaction([
        client.okxTradeJournal.deleteMany({ where: { tradeKey: { in: tradeKeys } } }),
        client.okxTrade.deleteMany({ where: { tradeKey: { in: tradeKeys } } }),
      ]);
      return res.count;
    },
