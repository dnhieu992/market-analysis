...
export type OkxTradeChartInput = {
  tradeKey: string;
  symbol: string;
  timeframe: string;
  url: string;
 ...
 * same trade+timeframe replaces the previous archive. See the `OkxTradeChart`
 * model in schema.prisma.
 */
export function createOkxTradeChartRepository(client = prisma) {
  return {
    /** All saved charts for one trade (any timeframe), newest first. */
    findByTradeKey(tradeKey: string) {
      return client.okxTradeChart.findMany({
        where: { tradeKey },
     ...
     */
    countBySymbol() {
      return client.okxTradeChart.groupBy({
        by: ['symbol'],
        _count: { _all: true },
      });
    },
    ...
    upsert(input: OkxTradeChartInput) {
      const { tradeKey, timeframe, symbol, url, objectKey, note = null } = input;
      return client.okxTradeChart.upsert({
