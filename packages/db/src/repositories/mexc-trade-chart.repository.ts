...
export type MexcTradeChartInput = {
  tradeKey: string;
  symbol: string;
  timeframe: string;
  url: string;
 ...
 * same trade+timeframe replaces the previous archive. See the `MexcTradeChart`
 * model in schema.prisma.
 */
export function createMexcTradeChartRepository(client = prisma) {
  return {
    /** All saved charts for one trade (any timeframe), newest first. */
    findByTradeKey(tradeKey: string) {
      return client.mexcTradeChart.findMany({
        where: { tradeKey },
     ...
     */
    countBySymbol() {
      return client.mexcTradeChart.groupBy({
        by: ['symbol'],
        _count: { _all: true },
      });
    },
    ...
    upsert(input: MexcTradeChartInput) {
      const { tradeKey, timeframe, symbol, url, objectKey, note = null } = input;
      return client.mexcTradeChart.upsert({
