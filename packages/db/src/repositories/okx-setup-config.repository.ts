...
export type OkxSetupConfigInput = {
  symbol: string;
  holdSide: string;
  leverage: number;
  marginUsd: number;
...
/**
 * Per-coin, per-side manual-open config for the /okx Setup tab. One row per
 ...
 * `OkxSetupConfig` model in schema.prisma.
 */
export function createOkxSetupConfigRepository(client = prisma) {
  return {
    /** Every saved config, for hydrating the whole Setup tab at once. */
    findAll() {
      return client.okxSetupConfig.findMany({
        orderBy: [{ symbol: 'asc' }, { holdSide: 'asc' }],
      });
    },

    /** Insert or update the config for one (symbol, holdSide). */
    upsert(input: OkxSetupConfigInput) {
      const { symbol, holdSide, leverage, marginUsd } = input;
      return client.okxSetupConfig.upsert({
     ...
     */
    upsertMany(inputs: OkxSetupConfigInput[]) {
      return client.$transaction(
        inputs.map(({ symbol, holdSide, leverage, marginUsd }) =>
          client.okxSetupConfig.upsert({
            where: { symbol_holdSide: { symbol, holdSide } },
            create: { symbol, holdSide, leverage, marginUsd },
            update: { leverage, marginUsd },
          }),
