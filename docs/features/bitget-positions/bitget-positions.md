...
3. Widget client `BitgetPositionsFeed` render 3 tile tổng hợp (Số dư tài khoản / Tổng ký quỹ / PnL chưa thực hiện — tile **"Vị thế đang mở"** đã bỏ 2026-07-27 vì trùng với số đếm trên nhãn tab) + bảng vị thế, và **tự làm mới mỗi 15 giây** qua `createApiClient().fetchBitgetPositions()` (dữ liệu authoritative: margin, realized PnL, vị thế mới/đã đóng); có nút "Làm mới" thủ công và mốc thời gian "đồng bộ … trước".
