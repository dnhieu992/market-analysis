...
5. **Toggle value:** state `showValue` (khởi tạo từ `localStorage` trong `useEffect` để tránh lệch SSR); khi tắt, tile "Số dư tài khoản" hiện `••••••` (kể cả dòng % vốn — biết % + vốn gốc là suy ra được số dư) và cột PnL + tile PnL chỉ hiện %/ROE; khi bật hiện số USD. Tile tổng ký quỹ render thẳng `fmtUsdPlain(...)`, không phụ thuộc `showValue`.
