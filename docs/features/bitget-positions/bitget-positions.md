...
**Số dư tài khoản:** tile **Số dư tài khoản** (equity = số dư ví + PnL chưa thực hiện) lấy từ `GET /api/v2/mix/account/accounts` (marginCoin USDT). Ngay dưới con số là **% thay đổi so với vốn gốc** — `(equity − vốn gốc) ÷ vốn gốc × 100`, xanh/đỏ theo dấu, kèm chú thích "so với vốn $X". Vốn gốc lấy từ **số dư danh mục `bitget` trên [/my-asset](../my-asset/my-asset.md)** — tổng mọi khoản vào trừ mọi khoản ra của bucket đó, nên nó **tự cập nhật** khi nạp/chuyển vốn, không phải sửa code hay env nữa. `BITGET_INITIAL_CAPITAL_USD` (mặc định **$100**) chỉ còn là fallback cho trường hợp danh mục `bitget` bị xoá khỏi /my-asset. Fetch song song với positions, non-fatal (lỗi → `null` → hiển thị "—", không làm trắng bảng). Tile **PnL chưa thực hiện** có cấu trúc giống tile số dư: dòng giá trị là **số USD** (ẩn thành `••••••` khi tắt value), và ngay dưới là **% luôn hiện** — tính trên **số tiền đang có = vốn + PnL đã chốt**, tức `totalUnrealizedPnlUsd ÷ (accountEquity − uPnL của snapshot)`, **không** tính trên vốn gốc. Mẫu số lấy `accountEquity` và `totalUnrealizedPnlUsd` từ **cùng một snapshot server** nên giá live chạy chỉ đổi tử số, mẫu số đứng yên; mẫu số ≤ 0 hoặc thiếu equity → hiện "—".

**Ẩn/hiện value:** toggle **👁 Hiện value / 🙈 Ẩn value** ở góc phải trên bảng áp dụng cho **số dư tài khoản** và **PnL**: khi tắt, tile "Số dư tài khoản" hiện `••••••` cho **số tiền USD**, còn dòng **% so với vốn gốc** thì **luôn hiện** (theo yêu cầu của user — lưu ý: biết % + vốn gốc là suy ra được số dư), và PnL (tile + cột từng dòng) chỉ hiện **%** (ROE / % trên "vốn + đã chốt"); khi bật hiện số USD đầy đủ. Riêng chú thích "so với vốn + đã chốt $X" của tile PnL chỉ kèm số USD khi bật value — nếu không sẽ lộ chính số dư đang ẩn. Lựa chọn lưu ở `localStorage` (`bitget:pnl-show-value`), mặc định **ẩn**. Nút toggle hiện bất cứ khi nào đã cấu hình API (kể cả khi không có vị thế nào) để luôn xem lại được số dư. Tile **Tổng ký quỹ** luôn hiện số USD.

**Tile Giá BTC:** cạnh tile PnL là tile **Giá BTC** (`BtcPriceTile`, dùng chung với `/mexc`) — giá BTC/USDT realtime + **% thay đổi trong 1h**. Giá lấy từ **chính WS của trang** (`BTCUSDT` được thêm vào danh sách subscribe nên không mở thêm kết nối, và trang vẫn LIVE kể cả khi không có vị thế nào). Mốc so sánh là **rolling 60 phút**: `GET /tracking-coins/coins/BTCUSDT/klines?interval=1m&limit=61` (proxy Binance server-side, tránh CORS/geo-block ở trình duyệt) → `open` của nến đầu tiên = giá đúng 60 phút trước; refetch mỗi 60s nên cửa sổ trượt liên tục. Đây **không phải** % kể từ đầu giờ. Giá nhấp nháy xanh/đỏ mỗi tick như cột giá trong bảng. Tile này **không bị ẩn** theo toggle value vì giá BTC là dữ liệu công khai.

**Lọc theo coin:** thanh công cụ ngay trên bảng có **danh sách chip, một chip mỗi coin đang mở vị thế** (`SymbolChipFilter` dùng chung với tab Lịch sử & Setup) — cho phép **chọn nhiều**, mặc định **không chip nào được chọn** (vẫn hiện toàn bộ vị thế). Bấm một chip để thu hẹp bảng theo coin đó (cộng dồn với các chip đã chọn); bấm lại để bỏ chọn. Có nút **✕ Xoá lọc** khi đang chọn ít nhất 1 chip. Bộ lọc chỉ thu hẹp **các dòng trong bảng**; 4 tile phía trên (số dư / ký quỹ / uPnL / giá BTC) và số đếm trên nhãn tab vẫn tính trên **toàn bộ** vị thế vì đó là số liệu cấp tài khoản. Hàng chip **luôn nằm một dòng, cuộn ngang** khi nhiều coin (modifier `.pf-coin-filter`, dùng chung với tab Lịch sử/Setup & Portfolio — 2026-09-13).

**Đặt TP/SL trên sàn:** mỗi dòng có nút **TP / SL** (ngay trước nút Đóng) hiện mức TP/SL đang live trên sàn. Bấm mở dialog để nhập giá kích hoạt; giá trị được đẩy thẳng lên Bitget dưới dạng **position TP/SL plan order** (`POST /api/v2/mix/order/place-pos-tpsl`, trigger theo **Mark Price**, đóng toàn bộ vị thế) — **sàn tự đóng lệnh khi chạm mức**, không phụ thuộc dashboard/worker có đang chạy hay không. Mức TP/SL hiện tại đọc từ **plan order đang chờ** (`orders-plan-pending?planType=profit_loss`, một call cho mọi coin, map theo `symbol|holdSide`: `pos_profit`→TP, `pos_loss`→SL, giữ order mới nhất theo `cTime`), **không** đọc từ `takeProfit`/`stopLoss` của `all-position` — hai field đó **rỗng với phần lớn lệnh SHORT và một số lệnh LONG dù TP/SL vẫn đang live trên sàn**, nên trước đây TP đã đặt không hiện lên bảng. `all-position` chỉ còn là fallback khi call plan-order lỗi (non-fatal, không làm trắng bảng). Mỗi lần đặt đều được ghi **log hệ thống** vào nhật ký lệnh (`kind: 'system'`).
...
3. Widget client `BitgetPositionsFeed` render 4 tile phía trên (Số dư tài khoản / Tổng ký quỹ / PnL chưa thực hiện / Giá BTC — tile **"Vị thế đang mở"** đã bỏ 2026-07-27 vì trùng với số đếm trên nhãn tab) + bảng vị thế, và **tự làm mới mỗi 15 giây** qua `createApiClient().fetchBitgetPositions()` (dữ liệu authoritative: margin, realized PnL, vị thế mới/đã đóng); có nút "Làm mới" thủ công và mốc thời gian "đồng bộ … trước".
...
- `apps/web/src/widgets/bitget/symbol-filter-input.tsx` — chip multi-select lọc coin (`SymbolChipFilter`) + hàm `matchesSymbolSelection`, dùng chung với tab Lịch sử và Setup.
...
- `apps/web/src/widgets/bitget-positions/use-bitget-live-prices.ts` — hook WebSocket public Bitget (ticker) trả map giá realtime + trạng thái kết nối. Danh sách subscribe **luôn có `BTCUSDT`** ngoài các symbol đang mở, để tile giá BTC có dữ liệu ngay cả khi không giữ vị thế nào.
- `apps/web/src/widgets/btc-price-tile/btc-price-tile.tsx` — tile **Giá BTC** dùng chung cho `/bitget` và `/mexc`: giá nhận từ WS của trang (prop `priceUsd`), mốc 1h lấy từ `GET /tracking-coins/coins/BTCUSDT/klines?interval=1m&limit=61` (làm mới mỗi 60s).
