...
9. **Bộ lọc coin & tile thống kê** (đồng bộ với `/bitget`, 2026-09-13) — trên cả 3 tab, hàng chip lọc coin **luôn một dòng, cuộn ngang** khi nhiều coin (modifier `.bg-toolbar-filter.pf-coin-filter`). Tab **Lịch sử**: khi chọn ≥1 coin, các tile thống kê **tính lại theo bộ lọc** (`displaySummary`) — tổng PnL ròng / win rate / số lệnh / TB-lệnh / lãi-lỗ lớn nhất, nhãn tile PnL đổi thành "Tổng PnL ròng (đã lọc)". Tab **Setup**: đã bỏ dòng mô tả "Mở lệnh nhanh…" và **đưa hai nút "+ Thêm coin" / "⚙ Setup nhiều coin" lên header** (cạnh ↻ Làm mới), toolbar chỉ còn bộ lọc coin.
...
- `apps/web/src/widgets/mexc/{setup-chart-dialog,bulk-setup-dialog,add-coin-dialog,chart-note-dialog,qqe-cell,star-rating,symbol-filter-input,chart-icon}.tsx` — UI phụ trợ của tab Setup (`symbol-filter-input` = bộ lọc coin dạng chip đa chọn, A→Z theo tên rút gọn — dùng chung với tab Vị thế và Lịch sử, giống hệt `/bitget`).
