...
SwingPaService
  ↓ fetch parallel:
    MarketDataService.getCandles('1d', 365)  → Binance klines
    MarketDataService.getCandles('1w', 150)  → Binance klines
    MarketDataService.getCandles('4h', 360)  → Binance klines
  ↓
...
| S/R zones | Lấy từ weekly candles (150 tuần), cluster 0.5%, zone width ±0.5% |
| Fibonacci | Đo retracement/extension từ swing gần nhất (swing high → swing low hoặc ngược lại) |
| Volume | Dùng để xác nhận breakout và liquidity sweep |
| Timeframe chính | Daily (1d) — 365 nến |
| Timeframe phụ | 4H — 360 nến — xác nhận pattern (pin bar, engulfing) |
...
Đo từ **swing gần nhất**:

- **Retracement** — từ swing high → swing low gần nhất (uptrend) hoặc swing low → swing high (downtrend)
  - Các mức quan trọng: 0.236, 0.382, 0.5, 0.618, 0.786
  - Dùng để xác định vùng pullback tiềm năng (entry confluence với S/R zone)
- **Extension** — từ swing low → swing high → retracement low (uptrend) hoặc ngược lại
  - Các mức quan trọng: 1.272, 1.414, 1.618
  - Dùng để xác định TP target khi giá breakout khỏi structure
...
| Fibonacci | Đường ngang từ swing gần nhất: retracement 0.236/0.382/0.5/0.618/0.786 (dashed), extension 1.272/1.618 (solid dash). Label góc trái. |
