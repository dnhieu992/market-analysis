...
**Lọc coin & phân trang:** trên bảng lịch sử có thanh công cụ với **danh sách chip, một chip mỗi coin xuất hiện trong lịch sử** (`SymbolChipFilter`, dùng chung với tab Setup & Vị thế — `apps/web/src/widgets/bitget/symbol-filter-input.tsx`) — cho phép **chọn nhiều chip cùng lúc**, mặc định **không chip nào được chọn** = xem tất cả; kèm số lệnh đang khớp và nút "✕ Xoá lọc" khi có ít nhất 1 chip đang chọn (bộ đếm "N lệnh (đã lọc từ M)" đã bỏ 2026-07-27 — số lệnh nằm trên nhãn tab). Bảng **phân trang client-side, mặc định 10 dòng/trang** (chọn 10/25/50/100), có nút Trước/Sau + "Trang x/y" + range "a–b / tổng". Lọc/đổi page-size tự đưa về trang 1. Toàn bộ lọc + phân trang chạy trên client (dữ liệu đã fetch, limit 200); các tile thống kê phía trên vẫn phản ánh **toàn bộ** lịch sử, không đổi theo bộ lọc.
