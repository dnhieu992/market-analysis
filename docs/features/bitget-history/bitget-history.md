...
3. **Web:** trang gộp `/bitget` (`BitgetPage`, server component) fetch song song positions + history rồi truyền vào `BitgetTabs`; tab **Lịch sử & PnL** render widget client `BitgetHistoryFeed` (chế độ `embedded`) — 6 tile thống kê + bảng lệnh, **tự làm mới mỗi 15s** (đọc DB, không gọi sàn) + nút "Làm mới". Cột **Chart** của mỗi lệnh chỉ có **một nút "📈 Xem chart"** (mở dialog review ở khung **H4** mặc định); người dùng đổi khung M30/H1/H4/D1 ngay trong dialog. Cột **Tham chiếu** có nút **"🖼 Reference"** mở gallery các chart **đã lưu từ đúng lệnh đó** (scope theo `tradeKey`), khác với tab Setup — gallery của Setup gom **toàn bộ chart đã lưu của cả một đồng coin** (scope theo `symbol`). Vào `/bitget?tab=history` để mở thẳng tab này; route cũ `/bitget-history` redirect sang đây.
...
- `apps/web/src/shared/api/client.ts` — `fetchBitgetHistory()`, `fetchBitgetSavedTradeCharts(tradeKey)` (Reference tab History), `saveBitgetTradeChart()`.
...
- `apps/web/src/widgets/bitget-history/bitget-history-feed.tsx` — widget: tile thống kê + bảng + auto-refresh 15s (prop `embedded`); thanh lọc coin `SymbolMultiSelect` + phân trang client-side (10/trang mặc định); nút Reference + `TradeChartGalleryDialog` (gallery scope theo `tradeKey`).
