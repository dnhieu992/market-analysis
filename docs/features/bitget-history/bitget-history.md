...
**Lọc coin & phân trang:** trên bảng lịch sử có thanh công cụ với **select-box multi-select coin name** (`SymbolMultiSelect`, dùng chung với tab Setup — `apps/web/src/widgets/bitget/symbol-multi-select.tsx`) liệt kê mọi coin đang có trong lịch sử — chọn nhiều coin để lọc, bỏ chọn hết = xem tất cả; kèm nút "✕ Xoá lọc" (bộ đếm "N lệnh (đã lọc từ M)" đã bỏ 2026-07-27 — số lệnh nằm trên nhãn tab). Bảng **phân trang client-side, mặc định 10 dòng/trang** (chọn 10/25/50/100), có nút Trước/Sau + "Trang x/y" + range "a–b / tổng". Lọc/đổi page-size tự đưa về trang 1. Toàn bộ lọc + phân trang chạy trên client (dữ liệu đã fetch, limit 200); các tile thống kê phía trên vẫn phản ánh **toàn bộ** lịch sử, không đổi theo bộ lọc.
