...
**Lưu & tham chiếu chart:** trong dialog "📈 Xem chart", nút **"💾 Lưu"** mở một **dialog ghi chú nhỏ** (`ChartNoteDialog`, dùng chung với tab Setup) cho phép nhập note (được để trống), rồi upload PNG lên R2 và lưu link kèm `tradeKey` + `note`. Note lưu ở cột `bitget_trade_charts.note` (nullable); gallery Reference hiển thị 📝 trên thumbnail có note và text đầy đủ dưới caption ảnh phóng to. Nút Reference của **tab History** đọc `GET /bitget/trade-chart/saved?tradeKey=` → chỉ ảnh của lệnh đó; nút Reference của **tab Setup** đọc `GET /bitget/trade-chart/by-symbol?symbol=` → mọi ảnh của coin đó. Cả hai dùng chung layout gallery (`.bg-gallery*`).
