...
**Lọc coin & phân trang:** trên bảng lịch sử có thanh công cụ với **ô lọc coin dạng free text** (`SymbolFilterInput`, dùng chung với tab Setup — `apps/web/src/widgets/bitget/symbol-filter-input.tsx`) — gõ tên coin, khớp substring không phân biệt hoa/thường, nhiều từ khoá cách nhau bằng dấu phẩy/khoảng trắng khớp bất kỳ từ nào; ô trống = xem tất cả; kèm số coin đang khớp, phím Esc và nút "✕ Xoá lọc" (bộ đếm "N lệnh (đã lọc từ M)" đã bỏ 2026-07-27 — số lệnh nằm trên nhãn tab). Bảng **phân trang client-side, mặc định 10 dòng/trang** (chọn 10/25/50/100), có nút Trước/Sau + "Trang x/y" + range "a–b / tổng". Lọc/đổi page-size tự đưa về trang 1. Toàn bộ lọc + phân trang chạy trên client (dữ liệu đã fetch, limit 200); các tile thống kê phía trên vẫn phản ánh **toàn bộ** lịch sử, không đổi theo bộ lọc.
