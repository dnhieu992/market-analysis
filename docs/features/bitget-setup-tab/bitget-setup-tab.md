...
3b. **Bulk setup:** User clicks **⚙ Setup nhiều coin** → dialog opens pre-ticked with the
   toolbar's currently selected coin chips (no chip selected = nothing pre-ticked). Filling the enabled side
...
- **First (cold) load after an API restart:** the QQE / change caches are empty, so that first
  request still fetches from Binance — but through the concurrency pool (8 at a time) rather than
  one call at a time, so it is far faster than before. Every load after that is served from memory
  (fresh) or from the stale copy while it refreshes in the background, so the columns fill in
  immediately even when the readings are due a refresh. A transient Binance failure keeps the
  last-known reading (the cache `at` is not bumped) so the next request retries rather than blanking.
...
- `apps/api/src/modules/bitget/bitget-setup-chart.service.ts` — fetches M30 Binance klines, builds open/closed position markers (via `BitgetService`), renders the chart PNG, computes the per-timeframe QQE column (`getQqeSignals`) and the 7d/30d/H4 change column (`getPriceChanges`). **Both columns are warmed the same way `/tracking-coins` warms its columns** (see `DailyCandleCacheService`): the per-(coin, tf) Binance klines are fetched **concurrently** through a `runPooled` pool of `FETCH_CONCURRENCY` (8) instead of one at a time (the old sequential nesting — ~40 coins × 4 QQE timeframes — was the Setup tab's main slowdown), each reading is **stale-while-revalidate** (served straight from memory under the 60s/5-min TTL, served from the stale copy while a background refresh runs up to `*_MAX_STALE_MS` = 6h, and only a missing/too-old reading makes the caller wait), and concurrent fetches of the same reading are de-duped via `qqeInFlight` / `changeInFlight`. `priceChangeFor` also fetches its daily klines and the H4 candle in parallel. The `/mexc` Setup tab's `MexcSetupChartService` carries the identical caching (7d/30d/90d, no H4).
- `apps/api/src/modules/bitget/bitget.controller.ts` — `GET /bitget/qqe-signals?symbols=…` returns the per-coin, per-timeframe QQE state for the Setup column. An optional `&timeframes=` (subset of `M30,1h,4h,1d,1w`) narrows the server-side scan; omitted here, so the Setup tab keeps the full M30/1h/4h/1d default. `/tracking-coins` passes `4h,1d,1w`. `GET /bitget/price-changes?symbols=…` returns the 7d/30d change ratio per coin. Both endpoints now carry `Cache-Control: private, max-age=60` (mirroring `/tracking-coins/scores` + `/price-changes`), so a reload inside a minute reuses the browser's copy.
