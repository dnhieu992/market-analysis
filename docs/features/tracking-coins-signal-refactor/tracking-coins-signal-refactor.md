...
- **Chart switcher = H4 / D1 / W1.** M15, M30 and H1 are gone; **weekly was added**.
  ...
  (H4/D1/W1) is the swing variant, used both by the chart switcher and by the QQE column scan —
  the Bitget Setup tab keeps its own M15…D1 set unchanged.
  Server side, `1w` was added to `TF_CONFIG` (limit 300 / display 80) and `TF_MS`, and `tfLabelOf`
  learned `1w → W1`.
  *(Fixed 2026-07-26: the prop was never actually passed at the call site, so the page still
  rendered the default M15…D1 switcher despite this doc.)*
