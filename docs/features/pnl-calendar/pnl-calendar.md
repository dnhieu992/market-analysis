...
## Edge Cases
- **Overview card vs this page** — the Total Profit / Loss card on `/` sums `closedPnlSum + Bitget + MEXC`, and this page sums the same three sources. They are computed independently, so an exchange added to one and not the other makes them disagree by exactly that exchange's realized total. Add a new exchange to both call sites in the same change. (OKX was a third exchange here until its page was removed on 2026-08-20; its trades no longer count anywhere.)
...
- `apps/web/src/app/pnl-calendar/page.tsx` — server-side fetch + merge of the four sources (manual orders, Bitget, MEXC, portfolio), resolves the initial tab
