...
sleep 1
pm2 start apps/api/dist/apps/api/src/main.js --name "$PM2_API" --cwd apps/api

echo "── Restart Worker"
pm2 delete "$PM2_WORKER" 2>/dev/null || true
pm2 start apps/worker/dist/apps/worker/src/main.js --name "$PM2_WORKER" --cwd apps/worker
