...
/**
 * Manual per-trade note timeline for a live MEXC position. Notes are grouped by
 ...
 */
@Injectable()
export class MexcJournalService {
  private readonly repo = createMexcTradeJournalRepository();

  private map(r: JournalRow): MexcJournalNoteDto {
    return {
      id: r.id,
      tradeKey: r.tradeKey,
      ...
      snapshot: (r.snapshot as MexcTradeJournalSnapshot | null) ?? null,
  ...
  async list(tradeKey: string): Promise<MexcJournalNoteDto[]> {
    const rows = await this.repo.findByTradeKey(tradeKey);
    return rows.map((r) => this.map(r));
  }

  async create(input: CreateJournalDto): Promise<MexcJournalNoteDto> {
  ...
  async update(id: string, input: UpdateJournalDto): Promise<MexcJournalNoteDto> {
