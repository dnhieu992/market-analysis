...
export class MexcController {
  constructor(
    private readonly service: MexcService,
    private readonly journal: MexcJournalService,
    private readonly setup: MexcSetupService,
    private readonly setupChart: MexcSetupChartService,
  ) {}

  @Get('positions')
  @ApiOperation({ summary: 'List all open positions on MEXC USDT futures' })
  getPositions() {
  ...
  @Post('positions/tpsl')
  @ApiOperation({
    summary: 'Set (or clear) the position TP/SL on MEXC so the exchange closes the position at the trigger',
  })
