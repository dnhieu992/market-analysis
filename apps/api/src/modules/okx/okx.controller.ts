...
import type { TradeChartParams } from './okx-setup-chart.service';
...
export class OkxController {
  constructor(
    private readonly service: OkxService,
    private readonly journal: OkxJournalService,
    private readonly setup: OkxSetupService,
    private readonly setupChart: OkxSetupChartService,
  ) {}

  @Get('positions')
  @ApiOperation({ summary: 'List all open positions on OKX USDT futures' })
  getPositions() {
  ...
  @Post('positions/tpsl')
  @ApiOperation({
    summary: 'Set (or clear) the position TP/SL on OKX so the exchange closes the position at the trigger',
  })
  ...
  @ApiOperation({ summary: 'Track a new coin in the Setup tab (validated against OKX contracts)' })
