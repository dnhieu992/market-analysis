...
import { OkxJournalDrawer, type JournalTarget } from '@web/widgets/okx-positions/okx-journal-drawer';
import type { OkxClosedTrade, OkxHistoryResponse, OkxTradeChart } from '@web/shared/api/types';

import { SymbolFilterInput, matchesSymbolQuery } from '@web/widgets/okx/symbol-filter-input';
import { ChartIcon } from '@web/widgets/okx/chart-icon';
import { ChartNoteDialog, ChartNoteView } from '@web/widgets/okx/chart-note-dialog';
...
type ChartTarget = { trade: OkxClosedTrade; tf: string };
...
export function OkxHistoryFeed({ initial, embedded = false, onCount }: Props) {
  const [data, setData] = useState<OkxHistoryResponse>(initial);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [journalTarget, setJournalTarget] = useState<JournalTarget | null>(null);
  const [chartTarget, setChartTarget] = useState<ChartTarget | null>(null);
  const [refTrade, setRefTrade] = useState<OkxClosedTrade | null>(null);
  ...
  const refreshChartCounts = useCallback(async () => {
    try {
      const rows = await clientRef.current.fetchOkxChartCountsByTrade();
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const next = await clientRef.current.fetchOkxHistory({ limit: 200 });
      setData(next);
      setError(null);
    } catch {
  ...
  return (
    <div className={embedded ? 'bg-panel' : 'page'}>
      <div className="bg-head">
        <div>
          {!embedded && <h1>OKX · Lịch sử lệnh &amp; PnL</h1>}
          <p className="bg-sub">
      ...
      {!configured && trades.length === 0 ? (
        <div className="bg-alert">
          ...
          <code>OKX_API_PASSPHRASE</code> vào <code>.env</code> để worker đồng bộ lịch sử lệnh.
        </div>
      ) : (
        <>
...
function tradeChartUrl(t: OkxClosedTrade, tf: string): string {
  ...
  return `${resolveApiBaseUrl()}/okx/trade-chart?${params.toString()}`;
}
...
function TradeChartDialog({
  ...
  async function save(note: string) {
    ...
    setSaveErr(null);
    try {
      const rec = await clientRef.current.saveOkxTradeChart({
 ...
 */
function TradeChartGalleryDialog({ trade, onClose }: { trade: OkxClosedTrade; onClose: () => void }) {
  const clientRef = useRef(createApiClient());
  ...
  useEffect(() => {
    ...
    setFailed(false);
    clientRef.current
      .fetchOkxSavedTradeCharts(trade.tradeKey)
      .then((list) => {
        if (!alive) return;
        setCharts(list);
