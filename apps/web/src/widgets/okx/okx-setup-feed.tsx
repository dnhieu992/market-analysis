...
import { useOkxLivePrices } from '../okx-positions/use-okx-live-prices';
import {
  DEFAULT_CHART_TF,
  SetupChartDialog,
  tfLabelOf,
...
export function setupSymbols(
  trades: OkxHistoryResponse['trades'],
  extra: string[] = [],
): string[] {
...
type ConfigMap = Record<string, OkxSetupConfig>;
...
/** Change-since-00:00-UTC ratio (0.0123) → signed percent string. */
...
export function OkxSetupFeed({
  ...
  const [positions, setPositions] = useState<OkxPositionsResponse>(initialPositions);
  ...
  const [watchlist, setWatchlist] = useState<OkxWatchlistSymbol[]>([]);
  ...
  const [priceChanges, setPriceChanges] = useState<Record<string, OkxPriceChange>>({});
  ...
  const addWatchlistSymbol = useCallback(async (symbol: string) => {
    const row = await clientRef.current.addOkxWatchlistSymbol(symbol);
  ...
  const removeWatchlistSymbol = useCallback(async (symbol: string) => {
    await clientRef.current.deleteOkxWatchlistSymbol(symbol);
  ...
  const refreshChartCounts = useCallback(async () => {
    try {
      const rows = await clientRef.current.fetchOkxChartCounts();
      const map: Record<string, number> = {};
  ...
  // Realtime last price + since-00:00-UTC change per coin, straight from OKX's public WS.
  const { prices: livePrices, changes: liveChanges, live } = useOkxLivePrices(symbols);

  // Sort value for a coin on a given column — stars from the saved priorities,
  // today from the live WS, 7d/30d/90d from the API. Returns null when a price reading
  ...
  const refreshQqe = useCallback(async (syms: string[]) => {
    ...
    try {
      const rows = await clientRef.current.fetchOkxQqeSignals(syms);
      setQqe((prev) => {
        const next = { ...prev };
  ...
  const refreshChanges = useCallback(async (syms: string[]) => {
    if (syms.length === 0) return;
    try {
      const rows = await clientRef.current.fetchOkxPriceChanges(syms);
  ...
  const saveBulkConfig = useCallback(
    async (input: { symbols: string[]; sides: BulkSideInput[] }) => {
      ...
      setNotice(null);
      try {
        const saved = await clientRef.current.saveOkxSetupConfigsBulk(input);
        setConfigs((prev) => {
          const next = { ...prev };
  ...
  const savePriority = useCallback(
    async (symbol: string, priority: number) => {
      ...
      setError(null);
      try {
        await clientRef.current.saveOkxSymbolPriority({ symbol, priority });
      } catch (err) {
  ...
  const openPosition = useCallback(
    async (symbol: string, holdSide: HoldSide) => {
      ...
      setNotice(null);
      try {
        const res = await clientRef.current.openOkxPosition({
          symbol,
          holdSide,
  ...
  return (
    <div className={embedded ? 'bg-panel' : 'page'}>
      <div className="bg-head">
        <div>
          {!embedded && <h1>OKX · Setup mở lệnh</h1>}
          <p className="bg-sub">
        ...
        <div className="bg-head-actions">
          <span className={`bg-live ${live ? 'bg-live--on' : ''}`} title="Giá realtime từ OKX WS">
      ...
      {!configured ? (
        <div className="bg-alert">
          ...
          <code>OKX_API_PASSPHRASE</code> vào <code>.env</code> để mở lệnh.
        </div>
      ) : symbols.length === 0 ? (
        <div className="bg-alert">
            ...
            <thead>
              <tr>
                ...
                <SortHeader
                  ...
                  sort={sort}
                  onSort={cycleSort}
                  title="Sắp xếp theo thay đổi so với mốc 00:00 UTC (OKX `sodUtc0`) — bấm để đổi chiều"
                />
                <SortHeader
                  label="7 ngày"
                  col="d7"
...
function ChartGalleryDialog({ symbol, onClose }: { symbol: string; onClose: () => void }) {
  const clientRef = useRef(createApiClient());
  const [charts, setCharts] = useState<OkxTradeChart[] | null>(null);
  ...
  useEffect(() => {
    ...
    setFailed(false);
    clientRef.current
      .fetchOkxSavedChartsBySymbol(symbol)
      .then((list) => {
        if (!alive) return;
        setCharts(list);
...
function SetupDialog({
  ...
  symbol: string;
  holdSide: HoldSide;
  initial: OkxSetupConfig | undefined;
