...
import { OkxHistoryFeed } from '@web/widgets/okx-history/okx-history-feed';
import { OkxPositionsFeed } from '@web/widgets/okx-positions/okx-positions-feed';
import { OkxSetupFeed, setupSymbols } from '@web/widgets/okx/okx-setup-feed';
import type { OkxHistoryResponse, OkxPositionsResponse } from '@web/shared/api/types';

export type OkxTab = 'positions' | 'history' | 'setup';
...
/**
 * Merged OKX dashboard: open positions + closed-trade history under one page,
 ...
 */
export function OkxTabs({ positions, history, initialTab = 'positions' }: Props) {
  const [tab, setTab] = useState<OkxTab>(initialTab);
  ...
  return (
    <div className="page">
      <h1>OKX · USDT Futures</h1>
      <div className="bg-tabs" role="tablist" aria-label="OKX views">
        <button
          type="button"
          role="tab"
      ...
      {tab === 'positions' ? (
        <OkxPositionsFeed initial={positions} embedded onCount={setPositionsCount} />
      ) : tab === 'history' ? (
        <OkxHistoryFeed initial={history} embedded onCount={setHistoryCount} />
      ) : (
        <OkxSetupFeed
          history={history}
          positions={positions}
