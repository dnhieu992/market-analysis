...
import type { OkxJournalNote, OkxPosition } from '@web/shared/api/types';
 ...
 */
export function tradeKeyOf(p: Pick<OkxPosition, 'symbol' | 'holdSide' | 'openedAt'>): string {
...
export function OkxJournalDrawer({ target, onClose }: Props) {
  const api = useMemo(() => createApiClient(), []);
  const { tradeKey, symbol, holdSide, status } = target;

  const [mounted, setMounted] = useState(false);
  const [notes, setNotes] = useState<OkxJournalNote[]>([]);
  ...
  function editNote(note: OkxJournalNote) {
  ...
  async function save() {
    ...
    try {
      ...
      if (editingId) {
        const updated = await api.updateOkxJournal(editingId, { content: finalContent, images });
        setNotes((prev) => prev.map((n) => (n.id === updated.id ? updated : n)));
      } else {
        const created = await api.addOkxJournal({
          tradeKey,
          symbol,
          holdSide,
