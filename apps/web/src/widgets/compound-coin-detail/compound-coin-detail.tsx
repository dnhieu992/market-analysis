...
type CompoundCoinDetailProps = Readonly<{
  portfolioId: string;
  coinId: string;
...
export function CompoundCoinDetail({ portfolioId, coinId, holding, transactions }: CompoundCoinDetailProps) {
  ...
  async function handleConfirmDelete() {
    if (!deleteId) return;
    try {
      await createApiClient().deleteCompoundTransaction(portfolioId, deleteId);
      setDeleteId(null);
  ...
  return (
    <main className="dashboard-shell">
      <div style={{ padding: '0.75rem 0 0' }}>
        <Link
          href={`/compound-interest/${portfolioId}`}
        ...
        <div className="dialog-backdrop" onClick={() => setAddOpen(false)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dialog-body">
              <CreateCompoundTransactionForm portfolioId={portfolioId} defaultCoinId={coinId} defaultPrice={currentPrice ?? undefined} onSubmitted={() => setAddOpen(false)} />
            </div>
          </div>
        </div>
      )}
