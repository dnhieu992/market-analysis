...
import { useMexcLivePrices } from './use-mexc-live-prices';
...
export function MexcPositionsFeed({ initial, embedded = false, onCount }: Props) {
  const [data, setData] = useState<MexcPositionsResponse>(initial);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError('Không tải được vị thế từ MEXC. Thử lại sau.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const closePosition = useCallback(
    async (symbol: string, holdSide: 'long' | 'short') => {
      ...
      setError(null);
      try {
        await clientRef.current.closeMexcPosition(symbol, holdSide);
        await refresh();
      } catch (err) {
  ...
  const saveTpsl = useCallback(
    async (
      ...
      setError(null);
      try {
        await clientRef.current.setMexcTpsl({ symbol, holdSide, ...input });
        setTpslKey(null);
        await refresh();
      } catch (err) {
  ...
  const lastJournalPos = useRef<MexcPosition | null>(null);
  ...
  return (
    <div className={embedded ? 'bg-panel' : 'page'}>
      <div className="bg-head">
        <div>
          {!embedded && <h1>MEXC · Vị thế đang mở</h1>}
          <p className="bg-sub">
      ...
      {!configured ? (
        <div className="bg-alert">
          ...
          <code>MEXC_API_PASSPHRASE</code> vào <code>.env</code> để xem vị thế.
        </div>
      ) : (
        <>
                ...
                <table className="bg-table">
                  <thead>
                    <tr>
                      ...
                      <th className="bg-num" title="TP/SL đặt trên sàn — MEXC tự đóng lệnh khi chạm mức (Fair Price)">
                        TP / SL
                      </th>
    ...
    <tr>
      ...
      <td className="bg-num">
        <button
          ...
          onClick={onTpsl}
          title="Đặt TP/SL trên sàn — MEXC tự đóng lệnh khi chạm mức (Fair Price)"
        >
