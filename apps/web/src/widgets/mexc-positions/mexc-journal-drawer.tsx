...
import type { MexcJournalNote, MexcPosition } from '@web/shared/api/types';
 ...
 */
export function tradeKeyOf(p: Pick<MexcPosition, 'symbol' | 'holdSide' | 'openedAt'>): string {
...
export function MexcJournalDrawer({ target, onClose }: Props) {
  const api = useMemo(() => createApiClient(), []);
  const { tradeKey, symbol, holdSide, status } = target;

  const [mounted, setMounted] = useState(false);
  const [notes, setNotes] = useState<MexcJournalNote[]>([]);
  ...
  function editNote(note: MexcJournalNote) {
  ...
  async function save() {
    ...
    try {
      ...
      if (editingId) {
        const updated = await api.updateMexcJournal(editingId, { content: finalContent, images });
        setNotes((prev) => prev.map((n) => (n.id === updated.id ? updated : n)));
      } else {
        const created = await api.addMexcJournal({
          tradeKey,
          symbol,
          holdSide,
  ...
  async function remove(id: string) {
    ...
    try {
      await api.deleteMexcJournal(id);
