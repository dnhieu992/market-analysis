...
type ChartTarget = { trade: MexcClosedTrade; tf: string };
...
type Props = {
  initial: MexcHistoryResponse;
  /** When rendered inside the merged MEXC tabs, drop the outer page chrome + title. */
  embedded?: boolean;
  /** Reports how many closed trades are loaded, so the tab label can show the count. */
  onCount?: (count: number) => void;
};

export function MexcHistoryFeed({ initial, embedded = false, onCount }: Props) {
  const [data, setData] = useState<MexcHistoryResponse>(initial);
  ...
  const refreshChartCounts = useCallback(async () => {
    try {
      const rows = await clientRef.current.fetchMexcChartCountsByTrade();
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const next = await clientRef.current.fetchMexcHistory({ limit: 200 });
      setData(next);
      setError(null);
    } catch {
  ...
  return (
    <div className={embedded ? 'bg-panel' : 'page'}>
      <div className="bg-head">
        <div>
          {!embedded && <h1>MEXC · Lịch sử lệnh &amp; PnL</h1>}
          <p className="bg-sub">
      ...
      {!configured && trades.length === 0 ? (
        <div className="bg-alert">
          ...
          <code>MEXC_API_PASSPHRASE</code> vào <code>.env</code> để worker đồng bộ lịch sử lệnh.
        </div>
      ) : (
        <>
...
function tradeChartUrl(t: MexcClosedTrade, tf: string): string {
  ...
  return `${resolveApiBaseUrl()}/mexc/trade-chart?${params.toString()}`;
}
...
function TradeChartDialog({
  ...
  async function save(note: string) {
    ...
    setSaveErr(null);
    try {
      const rec = await clientRef.current.saveMexcTradeChart({
 ...
 */
function TradeChartGalleryDialog({ trade, onClose }: { trade: MexcClosedTrade; onClose: () => void }) {
  const clientRef = useRef(createApiClient());
  ...
  useEffect(() => {
    ...
    setFailed(false);
    clientRef.current
      .fetchMexcSavedTradeCharts(trade.tradeKey)
      .then((list) => {
        if (!alive) return;
        setCharts(list);
