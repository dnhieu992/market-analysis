...
import type { BitgetPendingOrder, BitgetPosition, BitgetPositionsResponse } from '@web/shared/api/types';
...
export function BitgetPositionsFeed({ initial, embedded = false, onCount }: Props) {
  ...
  const [pending, setPending] = useState<BitgetPendingOrder[]>([]);
  ...
  const refreshPending = useCallback(async () => {
    try {
      const res = await clientRef.current.fetchBitgetPendingOrders();
  ...
  const cancelPending = useCallback(
    async (order: BitgetPendingOrder) => {
      if (
        !window.confirm(
      ...
      setError(null);
      try {
        await clientRef.current.cancelBitgetOrder(order.symbol, order.orderId);
...
function PendingOrdersPanel({
  ...
  disabled: boolean;
  onCancel: (order: BitgetPendingOrder) => void;
}) {
  return (
    <div className="bg-pending">
      <div className="bg-pending-head">
        ...
        <span className="bg-pending-hint">
          Chưa khớp · Bitget tự mở vị thế khi giá chạm mức · bấm Huỷ để gỡ
        </span>
      </div>
      <div className="bg-table-wrap">
        <table className="bg-table">
                ...
                <tr key={o.orderId}>
                  ...
                  <td className="bg-num">
                    <button
                      ...
                      onClick={() => onCancel(o)}
                      disabled={disabled}
                      title="Huỷ lệnh chờ này trên Bitget"
                    >
