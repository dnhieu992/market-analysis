...
/**
 * Fullscreen chart dialog for a MEXC coin (SonicR system + S/R channels + RSI,
...
export function SetupChartDialog({
  ...
  useEffect(() => {
    ...
    setSavedUrl(null);
    setSaveErr(null);
    const url = `${resolveApiBaseUrl()}/mexc/setup-chart?symbol=${encodeURIComponent(symbol)}&timeframe=${encodeURIComponent(tf)}&_t=${Date.now()}`;
  ...
  async function save(note: string) {
    ...
    setSaveErr(null);
    try {
      const rec = await clientRef.current.saveMexcSetupChart({ symbol, timeframe: tf, note });
      setSavedUrl(rec.url);
      setNotePrompt(false);
    } catch (err) {
