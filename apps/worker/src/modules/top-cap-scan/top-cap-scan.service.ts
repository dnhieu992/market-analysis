...
@Injectable()
export class TopCapScanService {
  private readonly logger = new Logger(TopCapScanService.name);
  ...
  async scanAll(): Promise<{ scanned: number; failed: number }> {
    ...
    this.logger.log(`Top-cap scan done — scanned: ${scanned}, failed: ${failed}`);
