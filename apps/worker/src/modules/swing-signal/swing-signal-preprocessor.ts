...
export function detectTrend(swings: SwingPoint[]): TrendResult {
...
export function analyzeVolume(candles: Candle[]): VolumeMetrics {
...
export function calculateAtr(candles: Candle[], period = ATR_PERIOD): AtrResult {
...
export function detectKeyLevels(
  ...
  const prices = swings.map((s) => ({ price: s.price, index: s.index, total: lookback.length }));

  // Cluster prices within 1.5%
...
export function calculateFibLevels(
  ...
  if (trend.direction === 'UPTREND') {
    ...
    // Find the most recent significant swing low then swing high
    for (let hi = highs.length - 1; hi >= 0; hi--) {
      const swingHigh = highs[hi]!;
      // Find a swing low before this high
  ...
  // SIDEWAYS: use recent major range
...
function buildTimeframeData(
  ...
  const { atr, atrPct } = calculateAtr(candles);
...
export function preProcess(
  ...
  // 24h change: last daily close vs previous
  ...
  return {
    ...
    weekly: buildTimeframeData(weeklyCandles, currentPrice, 20, true),
    daily: buildTimeframeData(dailyCandles, currentPrice, 60),
    fourHour: buildTimeframeData(fourHourCandles, currentPrice, 50)
  };
}
